...
f_catalog = # Halo directory here eg. '/.../halos/1/out_4_pid.list' 
...
for j in range(Grid):
    ...
    for i, (hx, hy, hz) in enumerate(pos_slide_halo_1):
        ...
        # density field shift of neutrinos to centre halo
        ...
        # rotate the neutrino density field with rotate function from scipy
    ...
    [ np.sin(angle_rad),  np.cos(angle_rad)]
        ...
        pos_yz = pos_slide_halo_1[:, [1,2]] - np.array([hy, hz])
        
        # Apply rotation in Y–Z plane
        pos_rotated_yz = pos_yz @ R
        
        # Reinsert X coordinate unchanged
        h_pos_rotate = np.copy(pos_slide_halo_1)
        h_pos_rotate[:, [1,2]] = pos_rotated_yz + np.array([hy, hz])        
    ...
    [ np.sin(angle_rad),  np.cos(angle_rad)]
        ...
        # Extract X,Z positions relative to the current halo center
        pos_xz = pos_slide_halo_2[:, [0,2]] - np.array([hx, hz])
        
        # Apply rotation in X–Z plane
        pos_rotated_xz = pos_xz @ R
        
        # Reinsert Y coordinate unchanged
        h_pos_rotate = np.copy(pos_slide_halo_2)
        h_pos_rotate[:, [0,2]] = pos_rotated_xz + np.array([hx, hz])        
        
        # shift halo position such that it is in the centre and halos that are moved off
        # one edge are located appropriately on the other side
        hx_shift = (h_pos_rotate[:,0]-hx+BoxSize/2)%BoxSize
        hz_shift = (h_pos_rotate[:,2]-hz+BoxSize/2)%BoxSize
        hx_shift = hx_shift//L
        hz_shift = hz_shift//L
        
        stacked_h_pos_x.append(hx_shift[k])
        stacked_h_pos_y.append(hz_shift[k])
    ...
    [ np.sin(angle_rad),  np.cos(angle_rad)]
        ...
        # Extract X,Y positions relative to the current halo center
        pos_xy = pos_slide_halo_3[:, [0,1]] - np.array([hx, hy])
        
        # Apply rotation in X–Y plane
        pos_rotated_xy = pos_xy @ R
        
        # Reinsert Z coordinate unchanged
        h_pos_rotate = np.copy(pos_slide_halo_3)
        h_pos_rotate[:, [0,1]] = pos_rotated_xy + np.array([hx, hy])
        ...
        stacked_h_pos_x.append(hx_shift[q])
        stacked_h_pos_y.append(hy_shift[q])
